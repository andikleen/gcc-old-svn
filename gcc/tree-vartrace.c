  ...
  if (dump_file)
    {
      ...
      print_generic_expr (dump_file, op, TDF_MEMSYMS);
      fprintf (dump_file, " type ");
      print_generic_expr (dump_file, TREE_TYPE (op), TDF_MEMSYMS);
      fputc ('\n', dump_file);
    }
}
...
/* Is OP something we can log?  Read/Write flag in FLAG. Logs
   temporaries if LOG_TEMPS is true.  */

static bool
supported_op (tree op, vartrace_flags flag, bool log_temps)
{
  enum attrstate s = neutral;
  do
    {
      s = enabled_op (op, s);
  ...
  if (DECL_P (op))
	...
	return false;
      if (s == force_on)
	return true;
      if (!DECL_EXTERNAL (op) && !TREE_STATIC (op) && !locals_enabled(flag))
	return false;
...
instrument_return (gimple_stmt_iterator *gi, greturn *gret)
{
  ...
  if (!rval)
    return false;
  // check if it may be already traced
