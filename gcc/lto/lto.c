...
static void iterative_hash_canonical_type (tree type, type_incr_hash &hstate);
...
hash_canonical_type (tree type)
{
  ...
  /* Combine a few common features of types so that types are grouped into
     ...
     checked.  */
  hstate.add_int (TREE_CODE (type) | (TYPE_MODE (type) << 16));
  ...
  if (INTEGRAL_TYPE_P (type)
      ...
      || POINTER_TYPE_P (type))
    {
      extra_bits = TYPE_UNSIGNED (type) | (TYPE_PRECISION (type) << 1);
      /* 11 bits used.  */
    }
  ...
  if (POINTER_TYPE_P (type))
    {
      hstate.add_int ((TYPE_ADDR_SPACE (TREE_TYPE (type)) << 16) |
...
static void
iterative_hash_canonical_type (tree type, type_incr_hash &hstate)
{
  hashval_t v;
