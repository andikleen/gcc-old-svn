...
class inchash_jhash
{
 public:
  ...
  inchash_jhash (hashval_t seed = 0)
  {
    val = seed;
  }
  ...
  void merge (inchash_jhash &other)
  {
  ...
  void add_commutative (inchash_jhash &a, inchash_jhash &b)
  {
    if (a.end() > b.end())
      {
        merge (b);
...
class inchash_spooky
{
 public:
  /* Start incremential hashing, optionally with SEED.  */
  inchash_spooky  (hashval_t seed = 0)
  {
   sp.Init (seed, seed);
  }
  ...
  /* Hash in state from other inchash OTHER.  */
  void merge (inchash_spooky &other)
  {
    uint64_t a, b;
    other.sp.Final (&a, &b);
    add_wide_int (a);
  ...
  void add_commutative (inchash_spooky &a, inchash_spooky &b)
  {
...
#define inchash_impl inchash_spooky
#else
#define inchash_impl inchash_jhash
#endif

/* Common utility functions for the inchash classes.  */

class inchash : public inchash_impl
{
 public:
  inchash(hashval_t seed = 0) : inchash_impl(seed), bits(0) {}
